...
contract FeeProxy is IOracle {
    ...
    function maxLiquidity(address token1, address token2) internal returns (uint24 fee, address poolAddress) {
        uint24 fee = 100;
        (uint balance, address poolAddresse) = getLiquidity(token1, token2, fee);
    
        for (uint i; i < 3; i++) {
            uint24 temp_fee = fees[i];
            (temp_balance, temp_address) = getLiquidity(token1, token2, temp_fee);
    ...
    function update(address token1, address token2) public {
        ...
        (param[tokenA][tokenB].poolAddress, param[tokenA][tokenB].fee) = maxLiquidity(tokenA, tokenB);
    }
